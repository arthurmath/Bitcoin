...
def main():
    ...
    # Configuration
    difficulte = 4  # Nombre de zéros requis au début du hash
    ...
    for i in range(2):
        ...
        bloc = mineur.miner_bloc(
            ...
            hash_dernier_bloc=blockchain.obtenir_dernier_bloc().hash,
    ...
    tx1 = Transaction(alice.adresse, bob.adresse, 1.5, alice.obtenir_cle_publique_hex())
    tx1.signer(alice)
    if tx1.est_valide():
        blockchain.ajouter_transaction(tx1)
        alice.solde_btc -= 1.5
        bob.solde_btc += 1.5
    
    # Transaction 2: Bob envoie 0.5 BTC à Charlie
    print("\n--- Transaction 2: Bob -> Charlie (0.5 BTC) ---")
    tx2 = Transaction(bob.adresse, charlie.adresse, 0.5, bob.obtenir_cle_publique_hex())
    tx2.signer(bob)
    if tx2.est_valide():
        blockchain.ajouter_transaction(tx2)
        bob.solde_btc -= 0.5
        charlie.solde_btc += 0.5
    
    # Transaction 3: Charlie envoie 1.0 BTC à Diana
    print("\n--- Transaction 3: Charlie -> Diana (1.0 BTC) ---")
    tx3 = Transaction(charlie.adresse, diana.adresse, 1.0, charlie.obtenir_cle_publique_hex())
    tx3.signer(charlie)
    if tx3.est_valide():
        blockchain.ajouter_transaction(tx3)
        charlie.solde_btc -= 1.0
        diana.solde_btc += 1.0
    
    # Transaction 4: Diana envoie 0.8 BTC à Alice
    print("\n--- Transaction 4: Diana -> Alice (0.8 BTC) ---")
    tx4 = Transaction(diana.adresse, alice.adresse, 0.8, diana.obtenir_cle_publique_hex())
    tx4.signer(diana)
    if tx4.est_valide():
        blockchain.ajouter_transaction(tx4)
        diana.solde_btc -= 0.8
        alice.solde_btc += 0.8
    ...
    tx5 = Transaction(alice.adresse, charlie.adresse, 2.0, alice.obtenir_cle_publique_hex())
    tx5.signer(alice)
    if tx5.est_valide():
        blockchain.ajouter_transaction(tx5)
        alice.solde_btc -= 2.0
        charlie.solde_btc += 2.0
    
    # Transaction 6: Bob envoie 1.0 BTC à Diana
    print("\n--- Transaction 6: Bob -> Diana (1.0 BTC) ---")
    tx6 = Transaction(bob.adresse, diana.adresse, 1.0, bob.obtenir_cle_publique_hex())
    tx6.signer(bob)
    if tx6.est_valide():
        blockchain.ajouter_transaction(tx6)
        bob.solde_btc -= 1.0
        diana.solde_btc += 1.0
    ...
    print("\nUtilisateurs:")
    for user in utilisateurs:
        print(f"  {user.nom}: {user.solde_btc:.4f} BTC")
    
    print("\nMineurs:")
    for mineur in mineurs:
        print(f"  {mineur.nom}: {mineur.solde_btc:.4f} BTC")
    ...
    print("🔗 Vous pouvez consulter ce fichier pour voir tous les détails\n")
